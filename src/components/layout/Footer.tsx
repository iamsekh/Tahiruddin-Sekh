    ...
    <footer className="relative bg-background overflow-hidden">
      ...
      <div className="container mx-auto px-6 max-w-7xl pt-16 pb-6">
        <div className="grid md:grid-cols-3 gap-12 items-start">
          ...
          <div>
            ...
            <div className="flex flex-col gap-2.5">
              {footerLinks.map(l => (
                <a key={l.label} href={l.href} className="text-sm font-medium text-zinc-400 hover:text-[#39ff14] transition-colors w-fit flex items-center gap-1 group">
                  {l.label}
            ...
            <div className="flex gap-2.5">
              {[
                ...
                <a key={href} href={href} target="_blank" rel="noopener noreferrer"
                  className="w-10 h-10 rounded-xl bg-black border border-zinc-800 flex items-center justify-center text-zinc-400 hover:bg-[#39ff14] hover:text-black hover:border-[#39ff14] transition-all duration-300 hover:-translate-y-0.5">
                  <Icon size={16} />
                </a>
              ))}
            </div>
        ...
        <div className="mt-16 pt-8 flex flex-col items-center justify-center gap-4 relative">
          ...
          <p className="text-[12px] text-zinc-500 font-medium relative text-center">© {new Date().getFullYear()} Tahiruddin Sekh. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
